...
def output_controller(recalls, precisions, average_precisions, num_lib, dictionary, cal_dect=False, cal_scale=False, path='train'):
    '''
    ...
    #     print('==========================================================================================================='        )
        ### Result Summary
    sum_lines = []
    sum_path = '/content/drive/MyDrive/fyp/obj_detection/yolo/Evaluation/'+path+'/sum.txt'
    fsumw = open(sum_path, 'a')
    if not cal_dect:
        # print('\n')
        # print(    'tag (have gt-boxes class):', tag)
        if tag > 0:
            line1 = '\t'.join('Sum #Anno:{}\tSum #Dects:{}\tSum #Hits:{}\t'.format(str(int(total_annos)), str(int(total_dects)), str(int(total_hits))))+'\n'
            print('Sum #Anno:{}\tSum #Dects:{}\tSum #Hits:{}\t'.format(str(int(total_annos)), str(int(total_dects)), str(int(total_hits))))
            line2 = '\t'.join('Total Precision:{}\tTotal Recall:{}\t'.format(str(round(total_hits/(total_dects+0.01), 3)), str(round(total_hits/total_annos, 3))))+'\n'
            print('Total Precision:{}\tTotal Recall:{}\t'.format(str(round(total_hits/(total_dects+0.01), 3)), str(round(total_hits/total_annos, 3))))
            sum_lines.append(line1)
            sum_lines.append(line2)
            if not cal_scale:
                line3 = '\t'.join('Whole mAP [{} type]:\t{}'.format(str(len(dictionary)), str(round(sumup/len(average_precisions),3))))+'\n'
                print('Whole mAP [{} type]:\t{}'.format(str(len(dictionary)), str(round(sumup/len(average_precisions),3))))
                sum_lines.append(line3)
                print('sum lines: ', sum_lines)
                # print('Non-sparse mAP [{} type]:\t{}'.format(str(int(tag)), str(round(sumup/tag,3))))
        else:
            line4 = '\t'.join('No ground-truth box. Skip it!') + '\n'
            sum_lines.append(line4)
            print('No ground-truth box. Skip it!')
        sum_lines.append('t'.join('one epoch end with if clause')+'\n')    
        print('===========================================================================================================')
    else:
        if tag > 0:
            print('Sum #Anno:{}\tSum #Dects:{}\tSum #Hits:{}\t'.format(str(int(total_annos)), str(int(total_dects)), str(int(total_hits))))
            print('Total Precision:{}\tTotal Recall:{}\t'.format(str(round(total_hits/(total_dects+0.01), 3)), str(round(total_hits/total_annos, 3))))
            line5 = '\t'.join('Sum #Anno:{}\tSum #Dects:{}\tSum #Hits:{}\t'.format(str(int(total_annos)), str(int(total_dects)), str(int(total_hits)))) + '\n'
            line6 = '\t'.join('Total Precision:{}\tTotal Recall:{}\t'.format(str(round(total_hits/(total_dects+0.01), 3)), str(round(total_hits/total_annos, 3)))) + '\n'
            sum_lines.append(line5)
            sum_lines.append(line6)
            if not cal_scale:
                print('Whole mAP [{} type]:\t{}'.format(str(len(dictionary)), str(round(sumup/len(average_precisions),3))))
                print('Non-sparse mAP [{} type]:\t{}'.format(str(int(tag)), str(round(sumup/tag,3))))
                line7 = '\t'.join('Whole mAP [{} type]:\t{}'.format(str(len(dictionary)), str(round(sumup/len(average_precisions),3)))) + '\n'
                line8 = '\t'.join('Non-sparse mAP [{} type]:\t{}'.format(str(int(tag)), str(round(sumup/tag,3)))) + '\n'
                sum_lines.append(line7)
                sum_lines.append(line8)
        else:
            print('No ground-truth box. Skip it!')
            line9 = '\t'.join('No ground-truth box. Skip it!') + '\n'
            sum_lines.append(line9)
        print('===========================================================================================================')
        sum_lines.append('t'.join('one epoch end with else clause')+'\n')  
    print(sum_lines)
    fsumw.writelines(sum_lines) 
    fsumw.close()
...
def compute_map(args):
    ...
    Map = output_controller(recalls=recalls, precisions=precisions, average_precisions=average_precisions, \
                      num_lib=num_lib, dictionary=process.dictionary, cal_dect=False, cal_scale=False, path=args.sum_path)
